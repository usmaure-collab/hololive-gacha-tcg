...
(async () => {
  let all = [];
  for(let i=1; i<=25; i++) {
    const res = await fetch('https://en.hololive-official-cardgame.com/cardlist/cardsearch_ex?expansion=hBP06&view=text&page='+i, {
  ...
  fs.writeFileSync('hbp06_scraped.json', JSON.stringify(all, null, 2));
