...
(async () => {
  let all = [];
  for(let i=1; i<=17; i++) {
    const res = await fetch('https://en.hololive-official-cardgame.com/cardlist/cardsearch_ex?expansion=hBP01&view=text&page='+i, {
  ...
  fs.writeFileSync('hbp01_scraped.json', JSON.stringify(all, null, 2));
