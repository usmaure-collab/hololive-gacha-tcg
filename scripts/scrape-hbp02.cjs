...
(async () => {
  let all = [];
  for(let i=1; i<=25; i++) {
    const res = await fetch('https://en.hololive-official-cardgame.com/cardlist/cardsearch_ex?expansion=hBP02&view=text&page='+i, {
      headers: {'User-Agent': 'Mozilla/5.0'}
    });
    const html = await res.text();
    const $ = cheerio.load(html);
    $('li.ex-item').each((_, el) => {
  ...
  fs.writeFileSync('hbp02_scraped.json', JSON.stringify(all, null, 2));
